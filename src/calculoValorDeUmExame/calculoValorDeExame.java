...
public class calculoValorDeExame {
    ...
    public calculoValorDeExame(int handle_convenio, int handle_exame, Date dataDoExame, boolean calculaValorDosMateriais, double descontoPaciente) {
        this.handle_convenio = handle_convenio;
        this.handle_exame = handle_exame;
        this.dataDoExame = dataDoExame;
        this.calculaValorDosMateriais = calculaValorDosMateriais;
    ...
    private void getValoresDosMateriais() throws SQLException{
        ...
        for (int i = 0; i < listaHandle_Material.size(); i++) {
           ...
           ResultSet resultSet = DAO.getConsultarValoresMateriaisDoExame(con, listaHandle_Material.get(i), dataDoExame);
           int j = 0; 
           while(resultSet.next()){
                //colocando dados nos objetos
    ...
    /*
     * Metodo que calcula o valor total dos materiais do exame (metodo chamado no metodo principal da clase (metodo principal é chamado das outras classes para calcular o valor deum exame)
     */
                ...
                for (int i = 0; i < listaHandle_Material.size(); i++) {
                    ...
                    if(listaValorMaterial.get(i)>=0){
                       //valor total eh igual a ele mesmo mais
                        ...
                        int handle_material = listaHandle_Material.get(i);
                        ...
                        BigDecimal valorTotalDoMaterial = new BigDecimal((valorMaterial * quantidadeMaterial)).setScale(2, RoundingMode.HALF_EVEN);
                        ...
                        String valorMaterialString = String.valueOf(new BigDecimal((valorMaterial)).setScale(2, RoundingMode.HALF_EVEN));
                        listaDeMateriais = listaDeMateriais + listaCodigoMaterial.get(i) + "&" + listaHandle_Material.get(i) + "&" + listaNomeMaterial.get(i) + "&" + valorMaterialString + "&" + quantidadeMaterial + "&" + valorTotalDoMaterial + "#";
                    }
    ...
    /*
     * metodo que verifica todos os materiais que estao sem valor e informa ao usuario
     */
    private void informarMateriaisSemValorAoUsuario(){
        //lista comos nomes dos valoresque nao foi achado algum valor (data invalida)
        List<String> listaNomeMaterialSemValor = new ArrayList<String>();
        
        //preenchendo a lsita comos materiais que nao tem valor (data invalida)
        for (int i = 0; i < listaValorMaterial.size(); i++) {
            if (listaValorMaterial.get(i)==-1.0) {
                listaNomeMaterialSemValor.add(listaNomeMaterial.get(i));
            }
        }
        
        //informando ao usuario os materiais que nao tem valor
        String materiaisSemValor = "";
        for (int i = 0; i < listaNomeMaterialSemValor.size(); i++) {
            if(i==0){
                materiaisSemValor = " - " + listaNomeMaterialSemValor.get(i) + "\n";
            }else{
                materiaisSemValor = materiaisSemValor + " - " + listaNomeMaterialSemValor.get(i) + "\n";
            }
            
        }
       

        //se a lista nao esta vazia mostra a mensagem ao usuario
        if(!listaNomeMaterialSemValor.isEmpty()){
            JOptionPane.showMessageDialog(null, "Os Materiais abaixo estão sem valor:\n" + materiaisSemValor + "Verifique a data consultada!"
    ...
    /*
     * Agora que temos o valor total dos materiais (ou 0 caso o ususario selecionou uma data que o material nao tinha valor e o usuario sera avisado!!!!!
     */
    /*
     * variaveis iniciais
     */
    ...
    /*
     * variavel referente a chConvenio * o ch1Exame
     * variavel referente a chConvenio * o ch2Exame
     * variavel referente a filmeConvenio * o filmeExame
     */
    ...
    /*
     * ESSE É O CARA
     * É ELE QUE CHAMAMOS PARA VERIFICA O VALOR DE UM EXAME E ELE CHAMARA TODOS OS OUTROS DESSA CLASSE
     * OS OUTROS METODOS SERVEM SOMENTE PARA DAR SUPORTE A ESTE
     * QUE É O QUE REALMENTE UTILIZAMOS QUANDO INSTANCIAMOS ESSA CLASSE!!!!!
     */
    ...
    private void calcularValorTotalDoExame(){
        ...
        if(calculaValorDosMateriais){
            try {
                ...
                JOptionPane.showMessageDialog(null, "Erro ao verificar Valor total dos Materiais. Procure o Administrador.","ERRO",javax.swing.JOptionPane.ERROR_MESSAGE);
        ...
        if(valorTotalMateriaisOk){
            try {
                ...
                JOptionPane.showMessageDialog(null, "Erro ao consultar Valor de Exame. Procure o Administrador.","ERRO",javax.swing.JOptionPane.ERROR_MESSAGE);
            }finally{
                Conexao.fechaConexao(con);
            }
        }
        ...
        /*
         * Agora que temos todas as variaveis (valores) necessarios para o calculo, vamos realizar a formula
         * apos fazer a valor teremos a variavel valorTotalDoExame com o valor, com este valor calculamos a parte do paciente e a parte do convenio (de acordo com as porcentagens pesquisadas no banco)
         */
        ...
        if(todosValoresOk){
            ...
            valorExame = new BigDecimal(ch1Total + ch2Total + filmeTotal + valorTotalMateriais).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
            valorConvenio = new BigDecimal(valorExame * (porcentConvenio/100)).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
            valorPaciente = new BigDecimal(valorExame * (porcentPaciente/100)).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
            
            
            valor_correto_convenio = new BigDecimal((valorExame - (valorExame * (redutor/100))) * (porcentConvenio/100)).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
            valor_correto_paciente = new BigDecimal((valorExame - (valorExame * (redutor/100))) * (porcentPaciente/100)).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
            
            //se o desconto for maior que 0 calculamos o desconto
            if (porcentDescontoPaciente > 0) {
                valor_desconto = new BigDecimal((valor_correto_paciente * (porcentDescontoPaciente/100))).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
                valor_correto_paciente = new BigDecimal(valor_correto_paciente - valor_desconto).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
            }else{
                valor_desconto = 0;
            }
            
            valor_correto_exame = new BigDecimal(valor_correto_paciente + valor_correto_convenio).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
        }
    }
