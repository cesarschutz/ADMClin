...
public class ImprimirNotaFiscalDoPacienteModelo2 {
    ...
    private void imprimirCabec(){
        ...
        imprimir.print("                                                              " + arrumarTamanhoDaString(dataDeHoje, 15));
        ...
        imprimir.print("        " + arrumarTamanhoDaString(paciente.getEndereco(), 45) + "    " + arrumarTamanhoDaString(paciente.getBairro(), 20));
        imprimir.lineFeed();
        ...
        imprimir.print("        " + arrumarTamanhoDaString(paciente.getCidade(), 25) + "     " + arrumarTamanhoDaString(paciente.getEstado(), 15) + "  " + arrumarTamanhoDaString(paciente.getCep(), 22));
        imprimir.lineFeed();
        ...
        imprimir.print("        " + arrumarTamanhoDaString(paciente.getCpf_cnpj(), 25) + "        " + arrumarTamanhoDaString(paciente.getInscricao_estadual(), 12) + "         " + arrumarTamanhoDaString("À VISTA", 15));
        imprimir.lineFeed();
        imprimir.lineFeed();
    ...
    private void imprimirExame(int i){
        //colocando o nome
        String nome_e_codigo = listaDeExames.get(i-1).getCod_exame() + " - " + listaDeExames.get(i-1).getModalidade() + " - " + listaDeExames.get(i-1).getNome();
        ...
        imprimir.print(arrumarStringAlinhadaADireita(MetodosUteis.colocarZeroEmCampoReais(String.valueOf(listaDeExames.get(i-1).getValor_correto_paciente())).replace(".", ","), 11));
        
        //imprimindo valor total do exame
        imprimir.print(arrumarStringAlinhadaADireita(MetodosUteis.colocarZeroEmCampoReais(String.valueOf(listaDeExames.get(i-1).getValor_correto_paciente())).replace(".", ","), 11));
        
        valor_total_da_nota = new BigDecimal(valor_total_da_nota + Double.valueOf(listaDeExames.get(i-1).getValor_correto_paciente())).setScale(2, RoundingMode.HALF_EVEN).doubleValue();
    ...
    private void imprimirTotal(int i) {
        ...
        String valorTotalDaNota = MetodosUteis.colocarZeroEmCampoReais(String.valueOf(valor_total_da_nota)).replace(".", ",");
        imprimir.print("                                                            " + arrumarStringAlinhadaADireita(valorTotalDaNota, 16));
    ...
    private void getDadosPaciente() throws SQLException{
        ResultSet resultSet = NotaFiscalDAO.getConsultarPaciente(con, handle_at);
    ...
    private void getExamesRealizados() throws SQLException{
        ResultSet resultSet = NotaFiscalDAO.getConsultarExames(con, handle_at);
        listaDeExames.clear();
        while(resultSet.next()){
