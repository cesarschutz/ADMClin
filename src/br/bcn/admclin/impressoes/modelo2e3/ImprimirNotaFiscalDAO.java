 ...
 * @author Cesar Schutz
 */
public class ImprimirNotaFiscalDAO {
    ...
    public static ResultSet getConsultarPaciente(Connection con, int handle_at) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select a.handle_paciente, p.nome, p.endereco, p.bairro, p.cidade, p.cep, p.uf, p.cpf, p.rg from atendimentos a "
                    + "inner join pacientes p on a.handle_paciente = p.pacienteid " + "where handle_at=?");
            stmtQuery.setInt(1, handle_at);
            resultSet = stmtQuery.executeQuery();
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Erro ao consultar os dados do Paciente. Procure o Administrador.",
