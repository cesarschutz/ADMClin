...
public class JFLaudo extends JFrame {
	 ...
	 * Create the frame.
	 */
	public JFLaudo(String data, String nomePaciente, String handle_at,
			String medico, String crmMedico, String mod) {
		...
		txtLaudo.setDocument(new br.bcn.admclin.ClasseAuxiliares.DocumentoSemAspasEPorcentoMinusculas(
				6144));
		...
		JBSalvar.setIcon(new ImageIcon(JFLaudo.class
				.getResource("/br/bcn/admclin/imagens/salvar.png")));
		...
		JBAssinar.setIcon(new ImageIcon(JFLaudo.class
				.getResource("/br/bcn/admclin/imagens/botaoAssinar.png")));
		...
		JBGerarPdf.setIcon(new ImageIcon(JFLaudo.class
				.getResource("/br/bcn/admclin/imagens/pdf.png")));
		...
		btnAjuda.setIcon(new ImageIcon(JFLaudo.class
	...
	private void iniciarClasse() {
		setIconImage(getToolkit().createImage(
				getClass().getResource(
						"/br/bcn/admclin/imagens/imagemIconePrograma.png")));
	...
	private boolean buscarInformacoesDoBanco() {
		try {
			...
			existeStudyDone = JLAUDOS
					.getConsultarStdAccession(handle_at_selecionado);
			radiologista = JLAUDOS
					.getConsultarRadiologista(handle_at_selecionado);
			return true;
		} catch (Exception e) {
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao consultar informações do Laudo. Procure o Administrador.",
	...
	private void buscaDadosPaciente(int handle_at) {
		...
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("SELECT PACIENTES.\"NASCIMENTO\" AS PACIENTES_NASCIMENTO, PACIENTES.\"SEXO\" AS PACIENTES_SEXO FROM \"PACIENTES\" PACIENTES INNER JOIN \"ATENDIMENTOS\" ATENDIMENTOS ON PACIENTES.\"PACIENTEID\" = ATENDIMENTOS.\"HANDLE_PACIENTE\" WHERE handle_at = ?");
	...
	private void botaoGerarPDF() {
		try {
			...
			criaPDFdoLaudo pdf = new criaPDFdoLaudo(txtHandle_at.getText(),
					txtDataAtendimento.getText(), txtPaciente.getText(),
		...
		if (USUARIOS.senhaAssinaturaDeLaudo.length() < 2) {
			...
			do {
				...
				int result = JOptionPane.showConfirmDialog(null, ob,
						"Defina sua senha para assinar laudos :",
