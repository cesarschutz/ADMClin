...
public class GerarArquivoTxtDaFatura {
    ...
    public GerarArquivoTxtDaFatura(String tp, Date dataInicial, Date dataFinal, String nome, int handle_convenio,
    ...
    public GerarArquivoTxtDaFatura(String tp, Date dataInicial, Date dataFinal, String nome, int grupo_id, int x,
        List<atendimentoModel> listaAtendimentos) {
        this.dataInicial = dataInicial;
        this.dataFinal = dataFinal;
    ...
    private void buscarAtendimentosNoBanco(int handle_at) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            if (!"".equals(listaMateriais) && !" ".equals(listaMateriais) && listaMateriais != null) {
                ...
                exame.setValor_materiais(new BigDecimal(valor_materiais * porcentagemConvenio).setScale(2,
    ...
    private boolean verificaSeProximoCabeNaNota(int index, int numeroLinha) {
        ...
        for (int i = index + 1; i < listaDeExames.size(); i++) {
            ...
            if (matricula2.equals(matricula1))
                n++;
            else
                break;
        }
    ...
    private void escrever() throws IOException {
        ...
        for (int i = 1; i <= numeroNota; i++) {
            for (int j = 0; j < listaDeExames.size(); j++) {
                ...
                if (Integer.valueOf(listaDeExames.get(j).getNn()) == i) {
                    ...
                    // fazendo a nota
                    nota =
                        nota + listaDeExames.get(j).getRef() + listaDeExames.get(j).getMatricula() + crmMedico
                            + listaDeExames.get(j).getDia() + listaDeExames.get(j).getCod_exame()
                ...
                // se a nota da lista for maior da que estamos montando ele da um break e vai para a proxima noto
                    ...
                    String valorTotalNotaString =
                        MetodosUteis.colocarZeroEmCampoReais(String.valueOf(new BigDecimal(valorTotalNota).setScale(2,
                            RoundingMode.HALF_EVEN).doubleValue()));
                    ...
                    String valorTotalMateriaisString =
                        MetodosUteis.colocarZeroEmCampoReais(String.valueOf(new BigDecimal(valorTotalMateriais)
        ...
        // arrumando o valor total da nota
        String valorTotalNotaString =
            MetodosUteis.colocarZeroEmCampoReais(String.valueOf(new BigDecimal(valorTotalNota - valorTotalMateriais)
                .setScale(2, RoundingMode.HALF_EVEN).doubleValue()));
        valorTotalNotaString = valorTotalNotaString.replace(".", "");
        if (valorTotalNotaString.length() < 13) {
            int falta = 13 - valorTotalNotaString.length();
        ...
        // arrumando o valor total de materiais
        String valorTotalMateriaisString =
            MetodosUteis.colocarZeroEmCampoReais(String.valueOf(new BigDecimal(valorTotalMateriais).setScale(2,
                RoundingMode.HALF_EVEN).doubleValue()));
        valorTotalMateriaisString = valorTotalMateriaisString.replace(".", "");
        if (valorTotalMateriaisString.length() < 9) {
            int falta = 9 - valorTotalMateriaisString.length();
    ...
    private boolean getNumeroNota() {
        try {
            ...
            JOptionPane.showMessageDialog(null, "Erro ao consultar número da nota. Procure o Administrador.", "Erro",
