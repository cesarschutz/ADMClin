...
import static br.bcn.admclin.janelaPrincipal.janelaPrincipal.internalFrameFinanceiroRelatorioFaturarConvenios;
...
public class jIFListaAtendimentosParaFaturar extends javax.swing.JInternalFrame {
    ...
    ImageIcon iconeSim = new javax.swing.ImageIcon(getClass().getResource("/menu/financeiro/relatorios/faturarConvenio/faturarConvenio/sim.png"));
    ImageIcon iconeSimPretoBranco = new javax.swing.ImageIcon(getClass().getResource("/menu/financeiro/relatorios/faturarConvenio/faturarConvenio/simPretoBranco.png"));
    ImageIcon iconeNao = new javax.swing.ImageIcon(getClass().getResource("/menu/financeiro/relatorios/faturarConvenio/faturarConvenio/nao.png"));    

    /*
     * metodo construtor por convenio
     * */
    public jIFListaAtendimentosParaFaturar(String tp, Date dataInicial, Date dataFinal, String nome, int handle_convenio, ArrayList<atendimentoModel> listaAtendimentos) {
        initComponents();
        iniciarClasse();
     ...
     */
    public jIFListaAtendimentosParaFaturar(String tp, Date dataInicial, Date dataFinal, String nome, int grupo_id, int x, ArrayList<atendimentoModel> listaAtendimentos) {
        initComponents();
        iniciarClasse();
        this.dataInicial = dataInicial;
        this.dataFinal = dataFinal;
    ...
    private void preenchendoTabela(){
        ...
        for (atendimentoModel atendimento : listaDeAtendimentos){
            modelo.addRow(new Object[]{atendimento.getHandle_at(), MetodosUteis.converterDataParaMostrarAoUsuario(atendimento.getData_atendimento().toString()), atendimento.getHora(), atendimento.getNomePaciente(), String.valueOf(atendimento.getFlag_laudo()), String.valueOf(atendimento.getFlag_faturado()), "0", "1"});
        }
        colocarIcones();
    ...
    private void abrirFinanceiroRelatorioFaturarConvenios(){
        this.dispose();
        br.bcn.admclin.janelaPrincipal.janelaPrincipal.internalFrameAtendimentosParaFaturar = null;
        ...
        try {
            ...
            if (geraArquivoTXT == 1) {
                ...
                int n = JOptionPane.showOptionDialog(null,
                        "Deseja gerar arquivo TXT?",
                        "Arquivo texto", JOptionPane.YES_NO_OPTION,
                ...
                if (n==0) {
                    ...
                    String mes2 = data2;
                    if (mes1.equals(mes2)) {
                        GerarArquivoTxtDaFatura gerar = new GerarArquivoTxtDaFatura("grupo",dataInicial, dataFinal, nome, grupo_id,0, listaDeAtendimentos);
                        gerar.criarTxt();
                    }else{
                        GerarArquivoTxtDaFatura gerar = new GerarArquivoTxtDaFatura("grupo",dataInicial, dataFinal, nome, grupo_id,0,listaDeAtendimentos);
                        gerar.criarTxt();
                        JOptionPane.showMessageDialog(null, "Atenção, você selecionou meses diferentes. O Arquivo TXT pode conter erros, para solucionar gere a fatura de um único mês.");
                    }
                }
            }
        } catch (Exception e) {
           ...
           ResultSet resultSet = br.bcn.admclin.dao.CONVENIO.getConsultarDadosDeUmConvenio(con,handle_convenio);
            try {
                ...
                if (geraArquivoTXT == 1) {
                    ...
                    String mes2 = data2;
                        if (mes1.equals(mes2)) {
                            GerarArquivoTxtDaFatura gerar = new GerarArquivoTxtDaFatura("convenio",dataInicial, dataFinal, nome, handle_convenio, listaDeAtendimentos);
                            gerar.criarTxt();
                        }else{
                            GerarArquivoTxtDaFatura gerar = new GerarArquivoTxtDaFatura("convenio",dataInicial, dataFinal, nome, handle_convenio, listaDeAtendimentos);
                            gerar.criarTxt();
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Selecione os exames para Faturar", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION));
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Código", "Data", "Hora", "Paciente", "Laudo", "Já Faturado", "Faturar", "faturar_numero"
            }
        ) {
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addComponent(jBGerarFatura, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
    ...
    private void jBGerarFaturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBGerarFaturaActionPerformed
        ...
        if (listaDeAtendimentos.size() > 0) {
            ...
            if (tipo.equals("grupo")) {
                faturaConvenio faturaConvenio = new faturaConvenio("grupo",dataInicial, dataFinal, nome, grupo_id,0, listaDeAtendimentos);
                if (faturaConvenio.gerarFatura()) {
                    gerarArquivoTextoGrupo();
                }
            }else{
                faturaConvenio faturaConvenio = new faturaConvenio("convenio", dataInicial, dataFinal, nome, handle_convenio, listaDeAtendimentos);
                if (faturaConvenio.gerarFatura()) {
            ...
            JOptionPane.showMessageDialog(br.bcn.admclin.janelaPrincipal.janelaPrincipal.internalFrameJanelaPrincipal, "Selecione um Atendimento.");
        }
