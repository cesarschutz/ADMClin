...
public class A_INTERVALOSPORHORARION {
    ...
    public static ResultSet getConsultar(Connection con) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Erro ao consultar Intervalos por Horário. Procure o Administrador.",
    ...
    public static boolean getConsultarParaSalvarRegistro(Connection con, A_intervalosPorHorarioN model) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,
                "Erro ao consultar se Intervalo por Horário já existe. Procure o Administrador.", "ERRO",
    ...
    public static boolean getConsultarParaAtualizarRegistro(Connection con, A_intervalosPorHorarioN model) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select * from A_INTERVALOSPORHORARION where nome=? and a_intervaloporhorarionid!=?");
            stmtQuery.setString(1, model.getNome());
    ...
    public static int getConsultarIdDeUmNomeCadastrado(Connection con, A_intervalosPorHorarioN model) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select a_intervaloPorHorarionid from a_intervalosPorHorarion where nome=?");
            stmtQuery.setString(1, model.getNome());
            resultSet = stmtQuery.executeQuery();
            ...
            JOptionPane.showMessageDialog(null,
                "Erro ao consultar ID do intervalo por Horário. Procure o Administrador.", "ERRO",
                javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
    ...
    public static ResultSet getConsultarDadosDeUmIntervaloPorHorario(Connection con, int intervaloPorHorarioId) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select * from a_intervalosporhorarion where a_intervaloporhorarionid=?");
            stmtQuery.setInt(1, intervaloPorHorarioId);
            resultSet = stmtQuery.executeQuery();
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Erro ao consultar Intervalo por Horário. Procure o Administrador.",
                "ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return resultSet;
        }
    ...
    public static boolean setCadastrar(Connection con, A_intervalosPorHorarioN model) {
        ...
        String sql =
            "insert into A_INTERVALOSPORHORARION (dat,usuarioid,nome,horarioinicial,horariofinal,seg,ter,qua,qui,sex,sab,dom) values(?,?,?,?,?,?,?,?,?,?,?,?)";
        try {
            ...
            conseguiuConsulta = false;
            JOptionPane.showMessageDialog(null, "Erro ao cadastrar Intervalo por Horário. Procure o Administrador.",
                "ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return cadastro;
        }
    ...
    public static boolean setDeletar(Connection con, int intervaloPorHorarioId) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Erro ao deletar Intervalo por Horário. Procure o Administrador.",
                "ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return deleto;
        }
    ...
    public static boolean setAtualizar(Connection con, A_intervalosPorHorarioN model) {
        boolean cadastro = false;
        String sql =
            "update A_INTERVALOSPORHORARION set dat=?, usuarioid=?, nome=?, horarioinicial=?, horariofinal=?, seg=?, ter=?, qua=?, qui=?, sex=?, sab=?, dom=? where A_INTERVALOPORHORARIONID=?";
        try {
            PreparedStatement stmt = con.prepareStatement(sql);
            stmt.setDate(1, model.getDat());
            stmt.setInt(2, model.getUsuarioId());
            ...
            conseguiuConsulta = false;
            JOptionPane.showMessageDialog(null, "Erro ao atualizar Intervalo por Horário. Procure o Administrador.",
                "ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return cadastro;
        }
