...
public class ATENDIMENTOS {
	...
	// variavel utilizada para quando SALVAR veriricar o handle_at que foi feito
	// naquele agendamento
	public static int handle_aTDoAgendamentoCadastrado;

	// recuperando o handle_at que sera utilizado para salvar (isso quando eh
	// feito uma reserva em algum horario)
	...
	public static ResultSet getConsultarAtendimentosAgenda(Connection con,
			int handle_agenda, Date dia) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select p.nome, a.handle_at, a.hora_atendimento, a.duracao_atendimento from atendimentos A inner join pacientes p on a.handle_paciente = p.handle_paciente where handle_agenda =? and data_atendimento=? order by hora_atendimento");
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao verificar os Atendimentos. Procure o Administrador.",
	...
	/**
	 * Consulta Todos os atendimentos desta agenda existentes no Banco de Dados
	 * que sejam reservas (o atendente ainda esta cadastrando o atendimento,
	 * esta com a janela atendimento daquele horario aberta)
	 * 
	 * @param Connection
	 * @return ResultSet
	 */
	@SuppressWarnings("finally")
	public static ResultSet getConsultarAtendimentosAgendaReservados(
			Connection con, int handle_agenda, Date dia) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select * from atendimentos where handle_paciente=? and handle_agenda=? and data_atendimento=?");
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao verificar os Atendimentos Reservados. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return resultSet;
		}
	...
	public static ResultSet getConsultarAtendimentos(Connection con, Date dia) {
		...
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select p.nome as nomePaciente, m.nome as nomeMedico, m.crm as crmMedico, a.handle_at, a.data_atendimento, a.hora_atendimento, a.status1, a.status2, a.EXAME_ENTREGUE_AO_PACIENTE, a.LAUDO_ENTREGUE_AO_PACIENTE, a.paciente_pagou from atendimentos A "
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao consultar os Atendimentos. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return resultSet;
		}
	...
	/**
	 * Consulta Todos os dados de um atendimento agenda existentes no Banco de
	 * Dados
	 * 
	 * @param Connection
	 * @return ResultSet
	 */
	@SuppressWarnings("finally")
	public static ResultSet getConsultarDadosDeUmAtendimento(Connection con,
			int handle_at) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select P.nome as nomepac, p.handle_paciente as handle_paciente, p.peso as peso, p.cidade as cidade, p.endereco as endereco_paciente, p.altura as altura, p.nascimento as nascimento_paciente, "
							...
							+ "inner join AREAS_ATENDIMENTO B on a.ID_AREAS_ATENDIMENTO = b.ID_AREAS_ATENDIMENTO "
							+ "inner join convenio c on c.handle_convenio = a.handle_convenio "
							+ "where a.handle_at = ?");
			stmtQuery.setInt(1, handle_at);
			resultSet = stmtQuery.executeQuery();
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(null,
					"Erro ao consultar Dados do Atendimento. Procure o Administrador."
	 ...
	 */
	@SuppressWarnings("finally")
    public static ResultSet getConsultarInformacoesCompletasAtendimento(Connection con, int handle_at) {
        ResultSet resultSet = null;
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select a.handle_at, a.data_entrega_exame, p.nome as nomePaciente, p.endereco as enderecoPaciente, p.bairro, p.cidade, p.cpf, m.nome as nomeMedico, m.crm, m.endereco as enderecoMedico, m.telefone, m.telefonedois, m.celular, u.nm_usuario, x.nm_usuario as usuarioEntregaExame "
                		+ "from atendimentos a "
	...
	public static boolean setCadastrar(Connection con, Atendimentos atendimento) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao registrar Atendimento. Procure o Administrador.",
					"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return cadastro;
		}
	...
	public static boolean setUpdate(Connection con, Atendimentos atendimento) {
		...
		try {
			...
			Date dataPagamentoPacienteAtual = new Date(
	...
	@SuppressWarnings("finally")
	public static boolean setUpdateQuandoForAtualizar(Connection con,
			Atendimentos atendimento) {
		boolean cadastro = false;
		String sql = "update atendimentos set data_atendimento=?, dat=?, data_exame_pronto=?, "
				+ "handle_paciente=?, handle_medico_sol=?, handle_convenio=?, hora_atendimento=?, duracao_atendimento=?, usuarioid=?, "
				+ "observacao=?, matricula_convenio=?, COMPLEMENTO=?, hora_exame_pronto=?, ID_AREAS_ATENDIMENTO = ?, paciente_pagou = ?, data_pagamento_paciente = ? where handle_at=?";
		try {
			...
			Date dataPagamentoPacienteAtual = new Date(
					new java.util.Date().getTime());
			stmt.setDate(16, dataPagamentoPacienteAtual);
	...
	public static boolean setDeletar(Connection con, int handle_at) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao deletar Atendimento. Procure o Administrador.",
					"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return deleto;
		}
	 ...
	 */
	@SuppressWarnings("finally")
	public static boolean setImpressoraNotaUtilizada(int handle_at,
	...
	/**
	 * marca flag_imprimiu como "S" para nao poder mais mecher nos valores do
	 * atendimento
	 * 
	 * @param Connection
	 * @param PacienteMODEL
	...
	public static boolean setUpdateFlagImprimiu(Connection con, int handle_at) {
		boolean cadastro = false;
		String sql = "update atendimentos set flag_imprimiu='S' where handle_at="
				+ handle_at;
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
			stmt.executeUpdate();
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao marcar flag de Impressão. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return cadastro;
		}
	 ...
	 */
	@SuppressWarnings("finally")
	public static boolean setUpdateStatus1(Connection con, int handle_at,
			String status) {
		boolean cadastro = false;
		String sql = "update atendimentos set status1=? where handle_at="
				+ handle_at;
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao mudar Status do Atendimento. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return cadastro;
		}
	...
	/**
	 * registra que apciente pagou o que esta pendente o flag 0 é nao e 1 eh sim
	 */
	public static boolean setPacientePagou(int handle_at, int flag) {
		boolean cadastro = false;
		Connection con = Conexao.fazConexao();
		String sql = "update atendimentos set paciente_pagou = ?, usuarioid = ?, data_pagamento_paciente = ?  where handle_at="
				+ handle_at;
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
			stmt.setInt(1, flag);
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao registrar pagamento de Paciente. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return cadastro;
		}
	...
	/**
	 * buscando o status 1 quando for imprimir!! se tive passado de 1 é pq nao
	 * fez algo depois de imprimir ficha e entao retorna false se for 1 retorna
	 * false tb pois ja esta o mesmo flag que iriamos marcar se for nulo ou
	 * vazio retonr tru para ele marcar o flag como 1 que foi impresso a ficha
	 * (ou seja paciente esta em atendimento, ainda nao mudo para exame ou laduo
	 * pronto!!
	 * 
	 * @param Connection
	 * @return Boolean
	...
	public static boolean getMarcarStatus1(Connection con, int handle_at) {
		...
		String sql = "select status1 from atendimentos where handle_at="
				+ handle_at;
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
	 ...
	 */
	@SuppressWarnings("finally")
	public static boolean setEntregouExameAoPaciente(Connection con,
			int handle_at) {
		boolean cadastro = false;
		String sql = "update atendimentos set exame_entregue_ao_paciente='S' where handle_at="
				+ handle_at;
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
			stmt.executeUpdate();
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao marcar Exame Entregue ao Paciente. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return cadastro;
		}
	...
	public static boolean setLaudoExameAoPaciente(Connection con, int handle_at) {
		boolean cadastro = false;
		String sql = "update atendimentos set laudo_entregue_ao_paciente='S' where handle_at="
				+ handle_at;
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
			stmt.executeUpdate();
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao marcar Laudo Entregue ao Paciente. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return cadastro;
		}
	...
	/**
	 * se vier no parametro 1 - entregou somente o exame se vier no parametro 2
	 * - entregou somente o laudo se vier no parametro 3 - entregou os dois
	 * 
	 * @param Connection
	 * @param PacienteMODEL
	 * @return Boolean
	...
	public static boolean setEntregaDeExame(Connection con, int parametro, int handle_at) {
		...
		if (parametro == 1) {
			...
			try {
				...
				stmt.executeUpdate();
				stmt.close();
				salvaDataEntregaExame(con, handle_at);
				cadastro = true;
			} catch (SQLException e) {
				JOptionPane
						.showMessageDialog(
	...
	private static void salvaDataEntregaExame(Connection con, int handle_at){
		String sql = "update atendimentos set data_entrega_exame = ? where handle_at="+ handle_at;
		try {
	 ...
	 */
	@SuppressWarnings("finally")
	public static ResultSet getConsultarModeloDeValidacaoMatriculaDoConvenio(
			Connection con, int handle_convenio) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select validacao_matricula from convenio where handle_convenio=?");
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao consultar Modelo de Validação da Matrícula. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return resultSet;
		}
	 ...
	 */
	@SuppressWarnings("finally")
	public static ResultSet getConsultarAgendaDeUmAtendimento(Connection con,
			int handle_at) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select a.handle_agenda, p.nome as nomepac from atendimentos as a "
							+ "inner join agendas p on a.handle_agenda = p.handle_agenda "
							+ "where a.handle_at=?");
			stmtQuery.setInt(1, handle_at);
			resultSet = stmtQuery.executeQuery();
		} catch (SQLException e) {
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao consultar Agenda do Atendimento. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return resultSet;
		}
	 ...
	 */
	@SuppressWarnings("finally")
	public static ResultSet getConsultarDadosEtiqueta(Connection con,
			int handle_at) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select a.data_atendimento, "
							+ "p.nome, "
			...
			JOptionPane
					.showMessageDialog(
							null,
							"Erro ao consultar informações para a Etiqueta. Procure o Administrador.",
							"ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
		} finally {
			return resultSet;
		}
	 ...
	 */
	@SuppressWarnings("finally")
	public static ResultSet getConsultarAtendimentosPorPeriodoEConvenio(
			Connection con, Date diaInicial, Date diaFinal,
			int handle_convenio, int handle_at) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
					.prepareStatement("select distinct a.handle_at, a.matricula_convenio, a.data_atendimento, c.porcentconvenio, c.handle_convenio, e.VALOR_CORRETO_CONVENIO, e.atendimento_exame_id, e.lista_materiais, p.nome, m.crm, t.cod_exame from atendimento_exames e "
	...
	@SuppressWarnings("finally")
	public static ResultSet getConsultarAtendimentosPorPeriodoEGrupo(
			Connection con, Date diaInicial, Date diaFinal, int grupo_id,
			int handle_at) {
		ResultSet resultSet = null;
		try {
			PreparedStatement stmtQuery = con
