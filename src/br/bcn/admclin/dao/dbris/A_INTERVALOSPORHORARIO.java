...
public class A_INTERVALOSPORHORARIO {
    ...
    public static ResultSet getConsultarAgendasDeUmIntervaloPorHorario(Connection con, int intervaloPorHorarioId) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select distinct a.agendaid, b.nome from a_intervalosporhorario as a inner join agendas as b on a.agendaid = b.agendaid  where a_intervalosporhorarionid = ?");
    ...
    public static ResultSet getConsultarIntervalosPorHorarioPorAgenda(Connection con, int agendaId) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select A_INTERVALOSPORHORARIO.agendaid, A_INTERVALOSPORHORARIO.A_INTERVALOSPORHORARIONID, A_INTERVALOSPORHORARION.A_INTERVALOPORHORARIONID, A_INTERVALOSPORHORARION.HORARIOINICIAL, "
                    ...
                    + "from A_INTERVALOSPORHORARIO inner join A_INTERVALOSPORHORARION on A_INTERVALOSPORHORARIO.A_INTERVALOSPORHORARIONID = A_INTERVALOSPORHORARION.A_INTERVALOPORHORARIONID where A_INTERVALOSPORHORARIO.agendaid ="
                    + agendaId);
            resultSet = stmtQuery.executeQuery();
        } catch (SQLException e) {
    ...
    public static boolean setCadastrar(Connection con, A_intervalosPorHorario model) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,
                "Erro ao cadastrar Agenda no Intervalo por Horário. Procure o Administrador.", "ERRO",
                javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return cadastro;
        }
