...
public class A_INTERVALOSPORPERIODO {
    ...
    public static ResultSet getConsultarAgendasDeUmIntervaloDiario(Connection con, int intervaloPorPeriodoNId) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select distinct a.agendaid, b.nome from a_intervalosporperiodo as a inner join agendas as b on a.agendaid = b.agendaid  where a_intervaloporperiodonid = ?");
    ...
    public static ResultSet getConsultarIntervalosPorPeriodoPorAgenda(Connection con, int agendaId) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select A_INTERVALOSPORPERIODO.agendaid, A_INTERVALOSPORPERIODO.A_INTERVALOPORPERIODONID, A_INTERVALOSPORPERIODON.A_INTERVALOPORPERIODONID, A_INTERVALOSPORPERIODON.HORARIOINICIAL, "
                    + "A_INTERVALOSPORPERIODON.HORARIOFINAL, A_INTERVALOSPORPERIODON.DIAINICIAL, A_INTERVALOSPORPERIODON.DIAFINAL "
                    + "from A_INTERVALOSPORPERIODO inner join A_INTERVALOSPORPERIODON on A_INTERVALOSPORPERIODO.A_INTERVALOPORPERIODONID = A_INTERVALOSPORPERIODON.A_INTERVALOPORPERIODONID where A_INTERVALOSPORPERIODO.agendaid ="
                    + agendaId);
            resultSet = stmtQuery.executeQuery();
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null,
                "Erro ao verificar os Intervalos por Período. Procure o Administrador.", "ERRO",
    ...
    public static boolean setCadastrar(Connection con, A_intervalosPorPeriodo model) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,
                "Erro ao cadastrar Agenda no Intervalo por Período. Procure o Administrador.", "ERRO",
                javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return cadastro;
        }
