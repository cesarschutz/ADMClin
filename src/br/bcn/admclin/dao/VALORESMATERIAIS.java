...
public class VALORESMATERIAIS {
    ...
    public static boolean getConsultarSeDataEhMenorQueAultimaCadastrada(Connection con, int handle_material,
        String dataAcadastrar, JTextField mensagemParaOUsuario) {
        boolean ok = false;
        ResultSet resultSet = null;
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select * from valoresmateriais where handle_material=? order by dataavaler");
    ...
    public static ResultSet getConsultar(Connection con, int handle_material) {
        ...
        try {
            PreparedStatement stmtQuery =
                con.prepareStatement("select * from valoresmateriais where handle_material=? order by dataavaler Desc");
            stmtQuery.setInt(1, handle_material);
            resultSet = stmtQuery.executeQuery();
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Erro ao consultar Valores do Material. Procure o Administrador.",
    ...
    public static boolean setCadastrar(Connection con, ValoresMateriais valorMaterialModel, int handle_material) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Erro ao cadastrar Valor de Material. Procure o Administrador.",
                "ERRO", javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return cadastro;
        }
    ...
    public static boolean setDeletar(Connection con, int handle_material) {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Erro ao deletar Valor de Material. Procure o Administrador.", "ERRO",
                javax.swing.JOptionPane.ERROR_MESSAGE);
        } finally {
            return deleto;
        }
